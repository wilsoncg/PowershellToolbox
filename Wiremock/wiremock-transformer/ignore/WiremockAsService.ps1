###############################################################################
#                                                                             #
#   File name       WiremockAsService.ps1                                             #
#                                                                             #
#   Description     A sample service in a standalone PowerShell script        #
#                                                                             #
#   Notes           The latest WiremockAsService.ps1 version is available in GitHub   #
...
#                   section below, or run: help .\WiremockAsService.ps1 -Detailed     #
#                                                                             #
#   History                                                                   #
...
<#
  ...
  .EXAMPLE
    ...
    C:\PS>.\WiremockAsService.ps1 -Status
    Not installed
    C:\PS>.\WiremockAsService.ps1 -Setup
    C:\PS># At this stage, a copy of WiremockAsService.ps1 is present in the path
  ...
  .EXAMPLE
    ...
    C:\PS># At this stage, no copy of WiremockAsService.ps1 is present in the path anymore
    C:\PS>.\WiremockAsService.ps1 -Status
    Not installed
...
$scriptName = $argv0.name               # Ex: WiremockAsService.ps1
$scriptFullName = $argv0.fullname       # Ex: C:\Temp\WiremockAsService.ps1

$scmStartArgs = Split-Path -Parent $PSCommandPath
Write-Verbose "scmStartArgs=$scmStartArgs"
Write-Verbose "runDir=$runDir"

# Global settings
$serviceName = $script                  # A one-word name used for net start commands
$serviceDisplayName = "Wiremock Service"
$ServiceDescription = "Wiremock running as a service"
  ...
  public class Service_$serviceName : ServiceBase { // $serviceName may begin with a digit; The class name must begin with a letter
    ...
    protected override void OnStart(string[] args) {
	  ...
	  if(imagePathArgs != null && 
		imagePathArgs.Length > 1)
	  {
		runDir = imagePathArgs[1];
	  }
	  var startMessage = string.Format("$exeName OnStart() // Entry. Starting script '$scriptCopyCname' -runDir \"{0}\" -Start", runDir);
      EventLog.WriteEntry(ServiceName, startMessage); // EVENT LOG
      ...
      try {
		...
		var pStartArgs = string.Format("-ExecutionPolicy Bypass -c & '$scriptCopyCname' -runDir \"{0}\" -Start", runDir);
        p.StartInfo.Arguments = pStartArgs; // Works if path has spaces, but not if it contains ' quotes.
        p.Start();
...
if ($Start) {                   # Start the service
  if ($isSystem) { # If running as SYSTEM, ie. invoked as a service
    ...
    Log "$scriptName -Start: IsSystem, Starting script '$scriptFullName' -Service"
	...
	Start-Process PowerShell.exe -ArgumentList ("-c & '$scriptFullName' -Service -runDir $runDir")
  } else {
...
if ($Setup) {                   # Install the service
  ...
  Write-Verbose "New-Service $serviceName -BinaryPathName '$exeFullName $scmStartArgs' -DisplayName $serviceDisplayName -Description $ServiceDescription -StartupType Automatic"
  $pss = New-Service $serviceName -BinaryPathName "$exeFullName $scmStartArgs" -DisplayName $serviceDisplayName -Description $ServiceDescription -StartupType Automatic
...
if ($Service) {                 # Run the service
  ...
  try {
	...
	$wmdir = $runDir
	Write-EventLog -LogName $logName -Source $serviceName -EventId 1001 -EntryType Information -Message  "wmdir = $wmdir"
	$commandPath = [Environment]::GetEnvironmentVariable('java_home') | Join-Path -ChildPath java.exe
	
	$wiremockArgs = "-cp $wmdir\wiremock-body-transformer-1.1.5.jar;$wmdir\wiremock-2.5.0-standalone.jar;$wmdir\slf4j-simple-1.7.16.jar com.github.tomakehurst.wiremock.standalone.WireMockServerRunner --verbose --extensions com.opentable.extension.BodyTransformer --root-dir $wmdir > $logDir\wiremock.log"
	
	Log "commandPath=$commandPath"
	Log "wiremockArgs=$wiremockArgs"
	
	Start-Process $commandPath -WorkingDirectory $wmdir -ArgumentList "$wiremockArgs"
	
	Log "Wiremock running on http://localhost:8080 (default)"
	Log "View mappings using swagger http://localhost:8080/__admin/swagger-ui/"
	Log "View wiremock log in $logDir\wiremock.log" 	
